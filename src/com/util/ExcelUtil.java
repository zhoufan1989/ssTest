...
public class ExcelUtil  {
	...
	private  DateFormat df_0 = new DateFormat("yyyy-MM-dd HH:mm:ss");
	private  DateFormat df_1 = new DateFormat("yyyy-MM-dd");
	private  DateFormat df_2 = new DateFormat("yyyy-MM");
	private  DateFormat df_3 = new DateFormat("yyyy");
     ...
     * @param list      数据源
     * @param fieldMap      类的英文属性和Excel中的中文列名的对应关系  key name-0, value 名称 
     * 								0 文本  1 数字  2 日期
     ...
     * @param dateType 日期转换格式 0 转换到秒  1转换到日  2转换到月 3转换到年
     * @param out       导出流
     * @throws Exception
     */
    public  <T>  void   listToExcel (
        ...
        if(list == null){
            throw new NullPointerException("数据源中没有任何数据");
        }

        if(sheetSize>65530 || sheetSize<1){
            sheetSize=65530;
        }
        ...
        try {
            ...
            for(int i=0; i<sheetNum; i++){
                ...
                if(1==sheetNum){
                    ...
                    fillSheet(sheet, list, fieldMap, 0, list.size()-1,datetype);
                ...
                //有多个工作表的情况
                    ...
                    //填充工作表
                    fillSheet(sheet, list, fieldMap, firstIndex, lastIndex,datetype);
                }
    ...
    public   <T>  void   listToExcel (
        ...
        listToExcel(list, fieldMap, sheetName, 65535, out,datetype);
    ...
    public   <T>  void   listToExcel (
        ...
        try {
            ...
            listToExcel(list, fieldMap, sheetName, sheetSize,out,dateType );
    ...
    public  <T>  void   listToExcel (
        ...
        listToExcel(list, fieldMap, sheetName,fileName , 65535, response, dateType);
    }
    ...
    /*<-------------------------辅助的私有方法----------------------------------------------->*/
    /**
     * @MethodName  : getFieldValueByName  对日期和小数进行转化
    ...
    private  <T> void fillSheet(
    	...
    	DateFormat df = null;
    	switch (datetype) {
		case 0:df = df_0;break;
		case 1:df = df_1;break;
		case 2:df = df_2;break;
		case 3:df = df_3;break;
		default:df = df_0;break;
		}
        ...
        for(int index=firstIndex;index<=lastIndex;index++){
            ...
            for(int i=0;i<enFields.length;i++){
            	String[] key = enFields[i].split("-");
                Object objValue=getFieldValueByNameSequence(key[0], item);
                String fieldValue=objValue==null ? "" : objValue.toString();
                WritableCell cell = null;
                String type ="";
                if(key.length == 2) type =key[1];
                switch (type) {
				case "0"://文本
					cell = new Label(i,rowNo,fieldValue,cellFormat2);
					break;
				case "1"://数值
					cell = new jxl.write.Number(i,rowNo,Double.parseDouble(fieldValue),cellFormat2);
					break;
				case "2"://日期
					cell =  new DateTime(i,rowNo, (Date) objValue, dateFormat);
					break;
				default:
					cell = new Label(i,rowNo,fieldValue,cellFormat2);
					break;
